...
type GrievanceThreadRouteProp = RouteProp<WorkerStackParamList, 'GrievanceThread'>;
type GrievanceThreadNavigationProp = StackNavigationProp<WorkerStackParamList, 'GrievanceThread'>;

interface GrievanceThreadProps {
  route: GrievanceThreadRouteProp;
  navigation: GrievanceThreadNavigationProp;
}

export default function GrievanceThread({ route, navigation }: GrievanceThreadProps) {
  const { threadId, supervisorName } = route.params;
  ...
  useEffect(() => {
    ...
    // Poll message list every 10 seconds for real-time chat simulation
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setReplyText(textToSend); // restore
    } finally {
      setSending(false);
    }
  };
  ...
  const canResolve = (role === 'ADMIN' || role === 'SUPERVISOR') && !isResolved;
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.headerTitle}>{supervisorName}</Text>
      ...
      {!isResolved ? (
        <View style={styles.inputBar}>
          <TextInput
            ...
            placeholder={t('grievances.replyPlaceholder')}
